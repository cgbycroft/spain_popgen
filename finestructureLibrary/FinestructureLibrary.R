...
require(ape)
require(XML)
source("~/Documents/ClareDPhil/DPhil/Spain/finestructureLibrary/FinestructureDendrogram.R") # minor modifications to dendrogram were needed
...
makemydend<- function(tdend,lablist,moresum=FALSE) {
	...
	if(length(test)>0) {
		...
		if(moresum){attr(testj[[i]],"label")<-NameMoreSummary(lablist[[test]])
		}else{ attr(testj[[i]],"label")<-NameSummary(lablist[[test]])}
		testj[[j]]<-tdend[[j]]
		tdend<-testj
		}
...
if(is.null(dend)){
	plot(c(0,1),c(0,1),type="n",axes=F,xlab="",ylab="",main=main)# null plot for top right
}else {
	plot.dendrogram(main=main,dend,horiz=FALSE,axes=FALSE,xaxs = "i", leaflab = "none",nodePar=nodePar,edgePar=edgePar)
}
...
print("using defaults")
	...
	if(tickmarks>0) {
		...
		axis(2,at=labelsaty,labels=rep("",length(labelsaty)),las=2,cex.axis=cex.axis,col.ticks=col.ticks)
	}
